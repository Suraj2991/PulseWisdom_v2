  ...
  private validateInsightUpdates(updates: Partial<InsightAnalysis>) {
    if (updates.insights) {
      ...
      updates.insights.forEach(insight => {
        if (!insight.id || !insight.type || !insight.description || !insight.category || 
            !insight.title || !insight.severity || !Array.isArray(insight.aspects) || 
            !Array.isArray(insight.houses) || !Array.isArray(insight.supportingFactors) || 
            !Array.isArray(insight.challenges) || !Array.isArray(insight.recommendations) || 
            !insight.date || !insight.createdAt || !insight.updatedAt) {
