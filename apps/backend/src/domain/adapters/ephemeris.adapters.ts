import { DateTime, GeoPosition, CelestialBody, BirthChart, Houses, Aspect } from '../types/ephemeris.types';
import { CelestialPosition, CelestialBody as EphemerisCelestialBody, AspectResponse, HouseResponse } from '../ports/IEphemerisClient';
...
export class EphemerisAdapter {
   ...
   */
  static toDomainCelestialBody(position: CelestialPosition, name: string, house: number = 1): CelestialBody {
    return {
   ...
   */
  static toDomainCelestialBodies(ephemerisBodies: EphemerisCelestialBody, houses: HouseResponse[]): CelestialBody[] {
  ...
  static toDomainHouses(houseResponses: HouseResponse[]): Houses {
    return {
      system: 'placidus', // This should be passed in from the service layer
